...
interface ScriptEditorWithTemplatesProps {
  script: Script | null;
  onSave: (script: Partial<Script>) => Promise<void>;
  onClose: () => void;
}

const ScriptEditorWithTemplates = ({ script, onSave, onClose }: ScriptEditorWithTemplatesProps) => {
  ...
  const [activeTab, setActiveTab] = useState<"editor" | "templates">(script ? "editor" : "templates");

  const { shareScript, isSharing } = useShareScript();
  ...
  const handleSelectTemplate = (template: { title: string; content: string; tags: string[] }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-full">
      {/* Left Panel - Editor or Templates */}
      <div className="flex-1 space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "editor" | "templates")}>
...
export default ScriptEditorWithTemplates;
